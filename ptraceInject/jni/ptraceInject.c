...
typedef struct {
    unsigned long start; // 模块起始地址
    unsigned long end;   // 模块结束地址
    int perms;           // 权限标志
    long offset;         // 文件偏移量
    long inode;          // inode节点号
    char dev[25];        // 设备信息
...
int ptrace_call(pid_t pid, uint32_t ExecuteAddr, long *parameters, long num_params,
    ...
    if (i < num_params) {
        ...
        if (ptrace_writedata(pid, (void *) regs->ARM_sp, (uint8_t * ) & parameters[i],
...
void *GetRemoteFuncAddr(pid_t pid, const char *ModuleName, void *LocalFuncAddr) {
    ...
    RemoteFuncAddr = (void *) ((long) LocalFuncAddr - (long) LocalModuleAddr +
                               (long) RemoteModuleAddr);
    return RemoteFuncAddr;
}

/*************************************************
 *  Description: 在指定进程的内存映射中列出具有特定名称的模块。该函数扫描 /proc/<pid>/maps 文件，以找到出现指定模块名称的内存区域。
 *  Input:       name - 指向字符字符串的指针，表示要在进程的内存映射中搜索的模块名称。
 *               pid  - 表示目标进程的进程ID的整数，其内存映射将被搜索。
 *  Return:      指向静态分配的 ProcMapInfo 结构体数组的指针，每个结构体包含一个与
 *               指定名称匹配的模块的信息。数组最多包含三个条目。如果没有找到匹配的模块，
 *               数组的内容是未定义的。调用者不应尝试释放返回的指针。
 * ***********************************************/
ProcMapInfo *ListModulesWithName(char *name, int pid) {
    ...
    if (fp != NULL) { // Use NULL instead of nullptr
        while (fgets(buffer, sizeof(buffer), fp) && i < 3) { // Check i to avoid overflow
            if (strstr(buffer, name)) {
                ProcMapInfo info = {0}; // Initialize the struct to zero
                char perms[10];
                char path[MAX_PATH];
                char dev[25];
                ...
                strncpy(info.dev, dev, sizeof(info.dev) - 1);
 ...
 * ***********************************************/
int inject_remote_process(pid_t pid, char *LibPath, char *FunctionName, long *FuncParameter,
                          long NumParameter) {
    int iRet = -1;
    struct pt_regs CurrentRegs, OriginalRegs;
    void *mmap_addr, *dlopen_addr, *dlsym_addr, *dlclose_addr, *dlerror_addr, *mprotect_addr, *memmove_addr, *mremap_addr;
    void *RemoteMapMemoryAddr, *RemoteModuleAddr, *RemoteModuleFuncAddr;
    long parameters[6];

    char *name = strrchr(LibPath, '/');
    if (name){
        name++;
    }else{
        name = LibPath;
    ...
    ProcMapInfo *maps = ListModulesWithName(name, pid);

    mprotect_addr = GetRemoteFuncAddr(pid, libc_path, (void *) mprotect);
    LOGD("mprotect RemoteFuncAddr:0x%lx", (long) mprotect_addr);

    memmove_addr = GetRemoteFuncAddr(pid, libc_path, (void *) memmove);
    LOGD("memmove RemoteFuncAddr:0x%lx", (long) memmove_addr);

    mremap_addr = GetRemoteFuncAddr(pid, libc_path, (void *) mremap);
    LOGD("mremap RemoteFuncAddr:0x%lx", (long) mremap_addr);
    ...
    for(int i = 0; i < 3; i++){
        ...
        {
            LOGD("Call Remote mmap Func Failed, err:%s\n", strerror(errno));
            ptrace_detach(pid);
            return iRet;
        }
        ...
        {
            LOGD("Removing protection: %s", maps[i].path);
            ...
            {
                LOGD("Call Remote mprotect Func Failed, err:%s\n", strerror(errno));
                ptrace_detach(pid);
                return iRet;
            }
        }
        ...
        {
            LOGD("Call Remote memmove Func Failed, err:%s\n", strerror(errno));
            ptrace_detach(pid);
            return iRet;
        }
        ...
        {
            LOGD("Call Remote mremap Func Failed, err:%s\n", strerror(errno));
            ptrace_detach(pid);
            return iRet;
        }
    ...
    if (ptrace_call(pid, (long) RemoteModuleFuncAddr, FuncParameter, NumParameter, &CurrentRegs) ==
        -1) {
...
*************************************************/
int
inject_remote_process_shellcode(pid_t pid, char *LibPath, char *FunctionName, long *FuncParameter,
                                long NumParameter) {
    int iRet = -1;
 ...
 _saved_cpsr_s, _saved_r0_pc_s;
    ...
    LOGD("ARM_r0:0x%lx, ARM_r1:0x%lx, ARM_r2:0x%lx, ARM_r3:0x%lx, ARM_r4:0x%lx, ARM_r5:0x%lx, ARM_r6:0x%lx, ARM_r7:0x%lx, ARM_r8:0x%lx, ARM_r9:0x%lx, ARM_r10:0x%lx, ARM_ip:0x%lx, ARM_sp:0x%lx, ARM_lr:0x%lx, ARM_pc:0x%lx", \
        CurrentRegs.ARM_r0, CurrentRegs.ARM_r1, CurrentRegs.ARM_r2, CurrentRegs.ARM_r3,
         CurrentRegs.ARM_r4, CurrentRegs.ARM_r5, CurrentRegs.ARM_r6, CurrentRegs.ARM_r7,
         CurrentRegs.ARM_r8, CurrentRegs.ARM_r9, CurrentRegs.ARM_r10, CurrentRegs.ARM_ip,
         CurrentRegs.ARM_sp, CurrentRegs.ARM_lr, CurrentRegs.ARM_pc);
    ...
    _inject_function_param_s = REMOTE_ADDR(inject_param_ptr, local_code_start_ptr,
                                           remote_code_start_ptr);
